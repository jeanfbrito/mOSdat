...
class VMOperations:
    ...
    def ensure_running(self, log_fn=print) -> bool:
        ...
        if not wait_for_ssh(self.vm.ip, self.vm.user, timeout=120):
    ...
    def deploy_package(self, package: Package, log_fn=print) -> str:
        ...
        result = self.ssh.scp_to(pkg_file, "/tmp/")
        if not result.success:
    ...
    def run_tests_no_gpu(self, package: Package, pkg_filename: str, log_fn=print) -> tuple[int, str]:
        log_fn(f"Running without-GPU tests for {package.format} on {self.vm.name}...")

        env = self._app_env(package, pkg_filename)
        cmd = f"{env} /tmp/tests/run-all.sh"
    ...
    def run_tests_gpu(self, package: Package, pkg_filename: str, log_fn=print) -> tuple[int, str]:
        log_fn(f"Running GPU tests for {package.format} on {self.vm.name}...")

        env = self._app_env(package, pkg_filename)
        cmd = f"{env} /tmp/tests/run-gpu-tests.sh"
