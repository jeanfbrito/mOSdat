...
function renderMatrix(){const rows=[];for(const run of state.runs||[])for(const vm of run.vms||[])rows.push(`<tr><td><div class="vm-name">${esc(vm.vm)}</div><div class="small">${esc(run.name)}</div></td><td><span class="pill ${cls(vm.status)}">${esc(vm.status)}</span></td><td>${vm.current_step?`step ${esc(vm.current_step.step_num)} <span class="small">${esc(vm.current_step.kind)}</span>`:'<span class="small">-</span>'}</td><td>${fmtAge(vm.duration_seconds)}</td><td><div class="step-row">${(vm.steps||[]).map(step=>`<button class="cell ${cls(step.status)}" title="step ${esc(step.step_num)} ${esc(step.status)}" onclick="openTimeline('${esc(run.name)}','${esc(vm.vm)}','${esc(step.step_num)}')">${esc(step.step_num)}</button>`).join('')}</div></td></tr>`);document.getElementById('matrix').innerHTML=rows.length?`<table><thead><tr><th>VM</th><th>Status</th><th>Current</th><th>Total runtime</th><th>Steps</th></tr></thead><tbody>${rows.join('')}</tbody></table>`:'<div class="empty">No functional runs found.</div>'}
