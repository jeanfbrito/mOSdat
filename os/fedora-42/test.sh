...
print_usage() {
    cat << 'EOF'
Run Wayland/X11 crash tests on Fedora 42 VM

Usage: ./test.sh [OPTIONS]

Options:
  --test NAME     Run specific test only:
                    wayland-real    - Real Wayland session
                    wayland-fake    - Fake Wayland socket (crash test)
                    wayland-nodisp  - Wayland session, no WAYLAND_DISPLAY
                    x11             - X11 session
                    all             - Run all tests (default)
  --output FILE   Write results to JSON file
  --label LABEL   Label for test run (e.g., version name)
  -h, --help      Show this help

Examples:
  ./test.sh                           # Run all tests
  ./test.sh --test wayland-fake       # Run specific test
  ./test.sh --output results.json     # Save results to JSON
EOF
}
...
log "========================================"
log "Running Tests on Fedora 42"
log "========================================"
log "VM ID:   ${VMID}"
log "VM IP:   ${VM_IP}"
log "Test:    ${TEST_NAME}"
...
run_test() {
    ...
    echo "=== Test: $name ==="
    echo "SESSION=$XDG_SESSION_TYPE WAYLAND=${WAYLAND_DISPLAY:-UNSET} DISPLAY=$DISPLAY"
    ...
    echo "$OUTPUT" | grep -q "ozone-platform=x11\|Forcing X11" && PLATFORM="X11-forced"
...
$(echo "$RESULTS" | while IFS=: read -r _ name result detail code platform; do
    echo "        {\"name\": \"$name\", \"result\": \"$result\", \"detail\": \"$detail\", \"exit_code\": $code, \"platform\": \"$platform\"},"
done | sed '$ s/,$//')
    ]
}
EOF
